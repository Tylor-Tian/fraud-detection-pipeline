...
class FraudDetectionSystem:
    ...
    def _calculate_location_risk(self, transaction: Transaction, 
        ...
        for known_loc in user_profile.locations:
            known_loc_dict = {
                'latitude': known_loc.latitude,
                'longitude': known_loc.longitude
            }
    ...
    def _detect_suspicious_pattern(self, transaction: Transaction, 
                                  features: Dict) -> bool:
        ...
        if features['current_velocity'] > 3:
            recent_merchants = self.storage.get_recent_merchants(
                transaction.user_id,
                limit=5
            )
    ...
    def _calculate_final_score(self, rule_flags: List[str], 
                              ml_score: float) -> Tuple[float, bool]:
    ...
    def _generate_explanation(self, features: Dict, flags: List[str], 
                             ml_score: float) -> Dict[str, float]:
